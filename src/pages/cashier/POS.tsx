...
function buildWhatsAppMessage(receipt: ReceiptData) {
  const items = receipt.items.map((item, index) => `${index + 1}. ${item.name}\n   ${item.qty} x ${formatCurrency(item.price)} = ${formatCurrency(item.price * item.qty)}`).join('\n');
  ...
  return [
    ...
    receipt.paymentMethod === 'CASH' ? `Uang Diterima: ${formatCurrency(receipt.cashReceived)}\nKembalian: ${formatCurrency(receipt.change)}` : 'Status: Lunas',
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-white/40">
              {cart.length} jenis item, {totalQty} pcs
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-3 sm:px-4">
        <div className="space-y-3">
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-1">
            <label className="block">
              <span className="mb-1.5 flex items-center gap-2 text-[11px] font-semibold text-white/55">
                <UserRoundCheck size={13} className="text-accent" /> Nama Kasir
              </span>
              <input
                value={cashierName}
                onChange={(event) => onCashierNameChange(event.target.value)}
                placeholder="Contoh: Siska"
                className="min-h-10 w-full rounded-xl border border-white/10 bg-dark/60 px-3 py-2 text-sm text-white outline-none placeholder:text-white/25 focus:border-accent/60"
              />
            </label>
            <label className="block">
              <span className="mb-1.5 flex items-center gap-2 text-[11px] font-semibold text-white/55">
                <User size={13} className="text-accent" /> Nama Pemesan
              </span>
              <input
                value={customerName}
            ...
            <label className="block sm:col-span-2 xl:col-span-1">
              <span className="mb-1.5 flex items-center gap-2 text-[11px] font-semibold text-white/55">
                <Phone size={13} className="text-accent" /> No. HP / WhatsApp
              </span>
              <input
                value={customerPhone}
          ...
          <label className="block">
            <span className="mb-1.5 flex items-center gap-2 text-[11px] font-semibold text-white/55">
              <StickyNote size={13} className="text-accent" /> Catatan Pesanan
            </span>
            <textarea
              value={note}
              onChange={(event) => onNoteChange(event.target.value)}
              rows={compact ? 2 : 2}
              placeholder="Contoh: tanpa gula tambahan, ambil jam 15.00"
              className="w-full resize-none rounded-xl border border-white/10 bg-dark/60 px-3 py-2 text-sm text-white outline-none placeholder:text-white/25 focus:border-accent/60"
            />
          </label>
                ...
                {cart.map((item) => (
                  <div key={item.id} className="rounded-2xl border border-white/10 bg-dark/45 p-3">
                    <div className="flex gap-3">
                      ...
                      <button type="button" onClick={() => onRemoveItem(item.id)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-red-500/10 text-red-300 hover:bg-red-500/20">
                        <Trash2 size={14} />
                      </button>
                    </div>
                    <div className="mt-3 flex items-center justify-between gap-3">
                      <div className="flex items-center rounded-xl border border-white/10 bg-surface">
                        <button type="button" onClick={() => onChangeQty(item.id, -1)} className="flex h-9 w-9 items-center justify-center text-white/60 hover:text-accent">
                          <Minus size={14} />
                        </button>
      ...
      <div className="shrink-0 border-t border-white/10 p-3 sm:p-4">
        ...
        <button
          ...
          className="flex min-h-11 w-full items-center justify-center gap-2 rounded-2xl bg-accent px-4 py-2.5 text-sm font-bold text-dark shadow-lg shadow-accent/20 transition-all hover:bg-cream active:scale-95 disabled:cursor-not-allowed disabled:opacity-40"
        >
          <Receipt size={17} /> Bayar & Buat Struk
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-dark">
        ...
        <div className="grid h-dvh min-w-0 gap-0 overflow-hidden xl:grid-cols-[minmax(0,1fr)_400px]">
          <section className="h-dvh min-w-0 overflow-y-auto overflow-x-hidden px-4 pb-28 pt-20 sm:px-5 lg:px-6 lg:py-6 xl:pb-8 xl:pr-5">
            ...
            <div className="mb-5 space-y-3">
              ...
              <div className="responsive-scroll-x -mx-1 px-1 pb-1">
                  ...
                  {PUBLIC_PRODUCT_CATEGORIES.map((category) => {
                    ...
                    return (
                      <button
                        ...
                        className={`rounded-2xl border px-4 py-2.5 text-left text-xs font-bold transition-all active:scale-95 ${isActive ? 'border-accent bg-accent text-dark shadow-lg shadow-accent/20' : 'border-white/10 bg-surface text-white/65 hover:border-accent/60 hover:text-accent'}`}
                      >
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 2xl:grid-cols-4">
                {products.map((product, index) => (
                  <motion.button
                    ...
                    className="group min-w-0 overflow-hidden rounded-2xl border border-white/10 bg-surface text-left shadow-xl shadow-black/15 transition-all hover:border-accent/60 hover:shadow-accent/10"
                  >
                    <div className="relative h-32 bg-cream sm:h-40">
                      <img
                        src={product.image}
                        alt={product.name}
                        width="420"
                      ...
                      />
                      <span className="absolute left-2 top-2 rounded-full bg-primary px-2 py-1 text-[10px] font-bold text-white shadow-lg">{product.tag}</span>
                    </div>
                    <div className="p-3 sm:p-4">
                      <p className="line-clamp-2 min-h-10 text-sm font-bold text-white">{product.name}</p>
                      <p className="mt-1 text-xs text-white/40">{product.categoryLabel}</p>
                      <p className="mt-2 text-sm font-bold text-accent">{formatCurrency(product.price)}</p>
                    </div>
                  </motion.button>
                ))}
              </div>
          ...
          <aside className="hidden h-dvh min-w-0 border-l-4 border-accent/80 bg-surface xl:block">
      ...
      <button
        ...
        className="fixed bottom-5 right-5 z-[65] flex min-h-14 min-w-14 items-center justify-center rounded-2xl bg-accent text-dark shadow-2xl shadow-black/40 ring-1 ring-white/10 xl:hidden"
      >
      ...
      <Modal open={cartOpen} onClose={() => setCartOpen(false)} title="Keranjang POS" size="xl">
        <div className="-m-4 max-h-[78dvh] sm:-m-5">
          <CartPanel
            compact
            cart={cart}
            totalQty={totalQty}
      ...
      <Modal open={paymentOpen} onClose={() => setPaymentOpen(false)} title="Pembayaran POS" size="lg">
        <div className="space-y-5">
          <div className="rounded-2xl border border-white/10 bg-dark/45 p-4">
            ...
            <p className="mt-2 text-3xl font-bold text-accent">{formatCurrency(total)}</p>
          </div>
          <div>
              ...
              {paymentOptions.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl border p-4 text-left transition-all active:scale-95 ${selected ? 'border-accent bg-accent text-dark shadow-lg shadow-accent/20' : 'border-white/10 bg-dark/45 text-white hover:border-accent/60'}`}
                  >
                    <Icon size={22} className={selected ? 'text-dark' : 'text-accent'} />
                    <p className="mt-3 font-bold">{option.label}</p>
                    <p className={`mt-1 text-xs leading-5 ${selected ? 'text-dark/65' : 'text-white/40'}`}>{option.desc}</p>
                  </button>
                );
              })}
            </div>
          ...
          {paymentMethod === 'CASH' && (
            <div className="rounded-2xl border border-white/10 bg-dark/45 p-4">
              <label className="block">
                <span className="mb-2 block text-sm font-semibold text-white">Uang Diterima</span>
                <input
                  value={cashReceived}
                  onChange={(event) => setCashReceived(event.target.value.replace(/[^0-9]/g, ''))}
                  placeholder="Masukkan nominal cash"
                  inputMode="numeric"
                  className="min-h-12 w-full rounded-xl border border-white/10 bg-surface px-4 py-3 text-white outline-none placeholder:text-white/25 focus:border-accent/60"
                />
              </label>
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="rounded-xl bg-surface p-3">
                  <p className="text-xs text-white/40">Total</p>
                  <p className="mt-1 font-bold text-white">{formatCurrency(total)}</p>
                </div>
                <div className="rounded-xl bg-surface p-3">
                  <p className="text-xs text-white/40">Kembalian</p>
                  <p className="mt-1 font-bold text-accent">{formatCurrency(changeAmount)}</p>
                </div>
              </div>
            </div>
          )}
          {paymentMethod !== 'CASH' && (
            <div className="rounded-2xl border border-mint/20 bg-mint/10 p-4 text-sm text-white/65">
              Pembayaran {getPaymentLabel(paymentMethod)} akan dikirim ke backend sebagai transaksi offline. Status berhasil hanya ditampilkan setelah backend mengembalikan transaksi.
            </div>
          )}
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <button type="button" onClick={() => setPaymentOpen(false)} disabled={submitting} className="rounded-xl border border-white/10 px-5 py-3 text-sm font-bold text-white/65 transition-colors hover:border-white/25 hover:text-white">
              Batal
            </button>
            <button
              type="button"
              ...
              disabled={submitting}
              className="rounded-xl bg-accent px-5 py-3 text-sm font-bold text-dark shadow-lg shadow-accent/20 transition-colors hover:bg-cream disabled:cursor-not-allowed disabled:opacity-60"
            >
      ...
      <Modal open={receiptOpen && !!receipt} onClose={() => setReceiptOpen(false)} title="Pembayaran Berhasil" size="lg">
        {receipt && (
          <div className="space-y-5">
            <div className="rounded-2xl border border-emerald-400/20 bg-emerald-950/30 p-4">
              <p className="text-sm font-semibold text-emerald-200">Transaksi berhasil diselesaikan.</p>
              <p className="mt-1 text-xs text-white/45">WhatsApp browser hanya bisa mengirim teks otomatis. Untuk foto struk, gunakan tombol bagikan/download PNG.</p>
            </div>
            <div className="rounded-3xl border border-white/10 bg-dark p-5">
              <div className="text-center">
                <p className="font-display text-2xl font-bold text-accent">Beard Papa's</p>
                <p className="mt-1 text-xs text-white/45">Fresh cream puff & desserts</p>
              </div>
              <div className="my-4 border-t border-dashed border-white/20" />
              <div className="grid gap-2 text-sm text-white/65 sm:grid-cols-2">
                <p>
                  <span className="text-white/35">No. Struk:</span> {receipt.invoice}
                </p>
                <p>
                  <span className="text-white/35">Tanggal:</span> {receipt.date}
                </p>
                <p>
                  <span className="text-white/35">Customer:</span> {receipt.customerName}
                </p>
                <p>
                  <span className="text-white/35">No. HP:</span> {receipt.customerPhone}
                </p>
                <p>
                  <span className="text-white/35">Kasir:</span> {receipt.cashier}
                </p>
                <p>
                  <span className="text-white/35">Metode:</span> {getPaymentLabel(receipt.paymentMethod)}
                </p>
                <p>
                  <span className="text-white/35">Tipe:</span> {receipt.orderType === 'DINE_IN' ? 'Dine In' : 'Take Away'}
                </p>
                {receipt.orderType === 'DINE_IN' && receipt.tableNumber && (
                  <p>
                    <span className="text-white/35">Meja:</span> {receipt.tableNumber}
                  </p>
                )}
              </div>
              <div className="my-4 border-t border-dashed border-white/20" />
              <div className="space-y-3">
                {receipt.items.map((item) => (
                  <div key={item.id} className="flex gap-3 text-sm">
                    ...
                    <p className="font-bold text-accent">{formatCurrency(item.price * item.qty)}</p>
                  </div>
                ))}
              </div>
              <div className="my-4 border-t border-dashed border-white/20" />
              <div className="space-y-2 text-sm">
                <div className="flex justify-between text-white/65">
                  <span>Total</span>
                  <span className="font-bold text-white">{formatCurrency(receipt.total)}</span>
                </div>
                {receipt.paymentMethod === 'CASH' ? (
                  <>
                    <div className="flex justify-between text-white/65">
              ...
              {receipt.note && (
                <div className="mt-4 rounded-2xl bg-surface p-3 text-sm text-white/60">
                  <span className="font-semibold text-white">Catatan:</span> {receipt.note}
                </div>
              )}
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <button
                type="button"
                onClick={() => openReceiptPrint(receipt)}
                className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-white/10 px-5 py-3 text-sm font-bold text-white/75 transition-colors hover:border-accent hover:text-accent"
              >
              ...
              <button
                ...
                className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-accent px-5 py-3 text-sm font-bold text-dark shadow-lg shadow-accent/20 transition-colors hover:bg-cream"
              >
                <Send size={17} /> Kirim WA Teks
              </button>
              <button
                type="button"
                onClick={() => downloadReceiptImage(receipt)}
                className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-accent/40 px-5 py-3 text-sm font-bold text-accent transition-colors hover:bg-accent hover:text-dark"
              >
                <Camera size={17} /> Download Foto Struk
              </button>
              <button type="button" onClick={handleShareImage} className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-surface px-5 py-3 text-sm font-bold text-white/75 transition-colors hover:bg-white/10">
                <Send size={17} /> Bagikan Foto
              </button>
            </div>
            <button
              type="button"
              ...
              className="flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-surface px-5 py-3 text-sm font-bold text-white/70 transition-colors hover:bg-white/10"
            >
              <X size={17} /> Transaksi Baru
            </button>
          </div>
        )}
      </Modal>
