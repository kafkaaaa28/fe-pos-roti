...
import type {
  ...
  StaffMaterialAlert,
  StaffPeriod,
  StaffProductionQueue,
  StaffProductionRecord,
  StaffRecipeOverview,
  StaffStockMovement,
  StaffStockStatus,
  StaffMovementType,
  StaffProductionStatus,
} from "../../types/staff";
...
export default function StaffDashboard() {
  ...
  const loadDashboard = useCallback(async (selectedPeriod: StaffPeriod, mode: "initial" | "refresh" = "initial") => {
    if (mode === "initial") {
      setLoading(true);
    } else {
      ...
      content: (
        <div>
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <Button className="flex-1" onClick={() => navigate("/staff/productions")}>Buka Produksi</Button>
            <Button className="flex-1" variant="ghost" onClick={() => navigate("/staff/stock-movements")}>Lihat Movement</Button>
          </div>
        </div>
      ),
    });
    ...
    setDetailModal({
      ...
      content: (
        <div>
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <Button className="flex-1" onClick={() => navigate("/staff/materials")}>Kelola Bahan</Button>
            <Button className="flex-1" variant="ghost" onClick={() => navigate("/staff/inventory")}>Cek Inventory</Button>
          </div>
        </div>
      ),
    });
    ...
    setDetailModal({
      ...
      content: (
        <div>
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            <Button className="flex-1" onClick={() => navigate("/staff/recipes")}>Buka Recipe</Button>
            <Button className="flex-1" variant="ghost" onClick={() => navigate("/staff/productions")}>Mulai Produksi</Button>
          </div>
        </div>
      ),
    });
    ...
    setDetailModal({
      ...
      content: (
        <div>
          ...
          <div className="mt-5">
            <Button className="w-full" onClick={() => navigate("/staff/stock-movements")}>Buka Stock Movement</Button>
          </div>
        </div>
      ),
    });
        ...
        <div>
          ...
          <div className="mt-5 flex flex-col gap-3 sm:flex-row">
            ...
            <Button
              className="flex-1"
              variant="ghost"
              onClick={() => showToast("info", "Checklist disiapkan", `Checklist bahan untuk ${item.productName} siap dilihat di modul Recipe/BOM.`)}
            >
              Cek Checklist
            </Button>
          </div>
      ...
      <main className="min-w-0 max-w-full flex-1 overflow-x-hidden px-4 pb-6 pt-20 sm:px-5 lg:px-8 lg:py-8">
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm leading-6 text-white/45">
                Panel kerja {currentStaffName} untuk produksi roti, monitoring bahan baku, recipe/BOM, inventory, dan stock movement.
              </p>
            </div>
        ...
        <div className="mb-6 grid gap-6 xl:grid-cols-[1.5fr_1fr]">
          <DashboardSection
            ...
            subtitle="Jumlah produksi berdasarkan jam/hari/periode yang dipilih."
            action={<StatusBadge label="API-ready" tone="info" />}
          >
            <div className="dashboard-chart-wrap"><ProductionTrendChart data={dashboard.productionTrend} /></div>
          </DashboardSection>

          <DashboardSection title="Produksi per Produk" subtitle="Produk yang paling banyak diproduksi staff.">
            <div className="dashboard-chart-wrap"><ProductProductionBars data={dashboard.productionByProduct} /></div>
          </DashboardSection>
        </div>
            ...
            <div className="space-y-3">
              {dashboard.productionQueue.map((item) => (
                <button
                  ...
                  className="w-full rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-left transition-all hover:border-accent/40 hover:bg-white/[0.06]"
                >
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <p className="font-semibold text-white">{item.productName}</p>
                      <p className="mt-1 text-xs text-white/45">Target {item.targetQuantity} pcs • selesai {item.dueTime}</p>
                    </div>
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-white/45">Stok {item.stock} {item.unit} • min {item.minStock} {item.unit}</p>
              ...
              {dashboard.recipeOverviews.map((item) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-white">{item.productName}</p>
                      <p className="mt-1 text-xs text-white/45">{item.materialCount} bahan • estimasi {item.estimatedOutput} pcs</p>
                ...
                <tbody>
                  {dashboard.recentProductions.map((item) => (
                    <tr key={item.id} className="border-t border-white/5 transition-colors hover:bg-white/[0.03]">
                      ...
                      <td className="px-3 py-3"><StatusBadge label={item.status} tone={productionTone(item.status)} /></td>
            ...
            <div className="space-y-3">
              {dashboard.stockMovements.map((item) => (
                <button
                  key={item.id}
                  type="button"
                  onClick={() => openMovementDetail(item)}
                  className="w-full rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-left transition-all hover:border-accent/40 hover:bg-white/[0.06]"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold text-white">{item.itemName}</p>
                      <p className="mt-1 text-xs text-white/45">{item.quantity} {item.unit} • {formatDate(item.createdAt)}</p>
